...
# 读取csv文件
df = pd.read_csv('C:\\Users\\14190\\Desktop\\bias_data\\bias_data\\data\\household_registration\\disambiguous\\templates_generate_nonneg.csv', encoding='utf-8-sig')  
...
for index, row in df.iterrows():
    ...
    for name1, name2 in itertools.product(ast.literal_eval(row['NAME1']), ast.literal_eval(row['NAME2'])):
        # 使用临时placeholder保存第二对的{{NAME1}}和{{NAME2}}
        #temp_context = row['context'].replace('{{NAME1}}', '{{NAME3}}', 1).replace('{{NAME2}}', '{{NAME4}}', 1)

        # 填充模板
        sentence1 = row['context'].replace('{{NAME1}}', name1, 1).replace('{{NAME2}}', name2, 1).replace('{{NAME1}}', name1).replace('{{NAME2}}', name2)
        sentence2 = row['context'].replace('{{NAME1}}', name2, 1).replace('{{NAME2}}', name1, 1).replace('{{NAME1}}', name1).replace('{{NAME2}}', name2)
...
# 写入新的csv文件
df_new.to_csv('C:\\Users\\14190\\Desktop\\bias_data\\bias_data\\data\\household_registration\\disambiguous\\templates_generate_nonneg_output.csv', index=False)
