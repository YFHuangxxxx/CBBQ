...
tokenizer = AutoTokenizer.from_pretrained("/data/hyf/glm/glm-10b", trust_remote_code=True)
model = AutoModelForSeq2SeqLM.from_pretrained("/data/hyf/glm/glm-10b", trust_remote_code=True).half().cuda()
model = model.eval()
...
with open("/data/hyf/glm/data/age/ambiguous.json", 'r', encoding='utf-8') as file:
    data = json.load(file)
...
for dic in data:
    ...
    final_df.to_csv("/data/hyf/glm/data/age/10b_result_amb.csv", mode='a', header=False)
    with open("/data/hyf/glm/data/age/10b_result_amb.csv", "a", encoding="utf-8") as f:
        f.write(answer + "\n")
