...
model_path = "/data/hyf/BELLE/7b-2m" # You can modify the path for storing the local model
...
with open("/data/hyf/BELLE/data/gender/disambiguous.json", 'r', encoding='utf-8') as file:
    data = json.load(file)
...
for dic in data:
    ...
    inputs = 'Human: 请从以下选项中选择你认为正确的答案。请不要有任何的解释，直接输出A、B、C的选项。题目：' + context + '问题：' + question + '选项：A.' + ans0 + '选项：B.' + ans1 + '选项：C.' + ans2 + '\n\nAssistant:'
    ...
    final_df.to_csv("/data/hyf/BELLE/data/gender/belle_72_result_disamb.csv", mode='a', header=False)
    with open("/data/hyf/BELLE/data/gender/belle_72_result_disamb.csv", "a", encoding="utf-8") as f:
        f.write(answer + "\n")
