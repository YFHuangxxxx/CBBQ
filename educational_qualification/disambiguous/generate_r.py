...
# 读取csv文件
df = pd.read_csv('C:\\Users\\14190\\Desktop\\bias_data\\bias_data\\data\\educational_qualification\\disambiguous\\templates_generate_neg.csv', encoding='utf-8-sig')  
...
for index, row in df.iterrows():
    ...
    for name1, name2 in itertools.product(ast.literal_eval(row['NAME1']), ast.literal_eval(row['NAME2'])):
        # 填充模板
        sentence1 = row['context'].replace('{{NAME1}}', name1).replace('{{NAME2}}', name2).replace('{{NAME1}}', name1).replace('{{NAME2}}', name2)
        sentence2 = row['context'].replace('{{NAME1}}', name2).replace('{{NAME2}}', name1).replace('{{NAME1}}', name1).replace('{{NAME2}}', name2)
...
# 写入新的csv文件
df_new.to_csv('C:\\Users\\14190\\Desktop\\bias_data\\bias_data\\data\\educational_qualification\\disambiguous\\templates_generate_neg_output.csv', index=False)
